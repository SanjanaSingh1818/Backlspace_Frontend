...
export default function AdminSignup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (secretKey !== ADMIN_SECRET) {
      setError("❌ Invalid secret key! Unauthorized access.");
      return;
    }
    ...
    try {
      setLoading(true);
      const res = await axios.post("https://backspace-fullstack.onrender.com/api/admin/register", {
        email,
        password,
        secretKey,
      });
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 pt-28 overflow-hidden bg-gradient-to-br from-gray-900 via-black to-gray-800 text-white">
      ...
      {/* 🔹 Floating geometric shapes with parallax motion */}
      {[...Array(10)].map((_, i) => (
        <motion.div
          key={i}
        ...
        <form onSubmit={handleSignup} className="space-y-3">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                required
                className="w-full pl-8 pr-2 py-1.5 bg-white/10 text-white placeholder-gray-400 border border-gray-600 rounded-md focus:ring-1 focus:ring-red-500 focus:border-transparent text-xs"
          ...
          <div>
            ...
            <input
              ...
              required
              className="w-full px-2 py-1.5 bg-white/10 text-white placeholder-gray-400 border border-gray-600 rounded-md focus:ring-1 focus:ring-red-500 text-xs"
          ...
          <p className="text-center text-[10px] text-gray-400 mt-2">
            Already an admin?{" "}
            <Link to="/admin" className="text-red-400 hover:text-red-500 font-medium">
              Sign In
            </Link>
          </p>
        </form>
